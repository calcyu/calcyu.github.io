...
> 如不想注册到服务中心，只需设置spring.cloud.service-registry.auto-registration.enabled=false ，或@EnableDiscoveryClient(autoRegister = false) 即可。
...
负载均衡：使多个并行的服务均匀的被调用，而不是全被集中在某一个服务上的解决方法
...
> eureka默认采用的是轮训负载RoundRobinRule

#### 修改负载规则

修改spring-cloud-consumer项目，
配置文件application.properties，增加属性
``` yml
