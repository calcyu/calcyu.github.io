---
title: Spring cloud（七）：zipkin链路跟踪
tag:
- java
- spring
- spring cloud
...
启动后浏览器中输入`http://localhost:9411/zipkin/`，进入zipkin后台管理界面。如图：

![]({{ "/assets/image/spring-cloud-zipkin/01.png" | prepend: site.baseurl }})

### 项目添加zipkin支持

分别在项目`spring-cloud-consumer`和`api-gateway`中添加zipkin的支持。
...
Spring Cloud Sleuth有一个Sampler策略，可以通过这个实现类来控制采样算法。采样器不会阻碍span相关id的产生，但是会对导出以及附加事件标签的相关操作造成影响。 Sleuth默认采样算法的实现是Reservoir sampling，具体的实现类是PercentageBasedSampler，默认的采样比例为: 0.1(即10%)。不过我们可以通过spring.sleuth.sampler.probability来设置，所设置的值介于0.0到1.0之间，1.0则表示全部采集。

这两个项目添加zipkin之后，依次进行启动。

> 如果希望在控制台打印跟踪信息，可以修改日志级别，在application.properties中增加配置

**spring-cloud-consumer项目**
...
四个项目均启动后，在浏览器中访问地址：`http://localhost:8888/spring-cloud-consumer/hello/1111` 两次，然后再打开地址：
`http://localhost:9000/zipkin/`点击对应按钮进行查看。

点击查找看到有两条记录

![]({{ "/assets/image/spring-cloud-zipkin/02.jpg" | prepend: site.baseurl }})  

点击记录进去页面，可以看到每一个服务所耗费的时间和顺序

![]({{ "/assets/image/spring-cloud-zipkin/03.jpg" | prepend: site.baseurl }})  

点击依赖分析，可以看到项目之间的调用关系

![]({{ "/assets/image/spring-cloud-zipkin/04.jpg" | prepend: site.baseurl }}) 


**[示例代码-github](https://github.com/calcyu/spring-cloud-examples/tree/master/Lesson06)**
